...
1. **Apple Developer Program** — **Done.** The organization membership is active.
...
7. **Expo account & EAS project** — **Done.** Linked to `@amaroksoftware/amarok-one-mobile`.
...
Run `pnpm release:check:ios` during development and the strict
`EXPO_PUBLIC_API_URL=https://your-staging-api pnpm release:check:ios:staging` before uploading a
TestFlight build.
