...
export const authRoutes = new Hono()
  ...
  .post("/logout", jwtGuard, tenantContextMiddleware, async (context) => {
  ...
  .get("/me", jwtGuard, tenantContextMiddleware, async (context) => {
    const auth = getAuth(context);
    const user = await getCurrentUser(auth.user.sub, auth.user.organizationId);
